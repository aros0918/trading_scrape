...
with open('difference.txt', 'r') as file:
    ...
    #     print(url)
with open('complete (2).csv', 'r', encoding='latin-1') as file:
    csv_reader = csv.reader(file)
    first_column = [row[0] for row in csv_reader]
print(len(set(first_column)))
difference = list(set(splited_data) - set(first_column))
print(len(difference))
with open('difference1.txt', 'w') as file:
    for _ in difference:
        file.write(_+'\n')
...
#     with open('difference.txt', 'r') as file1:
#         content = file1.read()
#         splited_data = content.splitlines()
#         for url in splited_data:
#             driver.get(url)
